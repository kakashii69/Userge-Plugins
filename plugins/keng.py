...
@userge.on_cmd(
    "keng", about={
        ...
        'usage': "Reply {tr}keng [emoji('s)] [pack number] to a sticker or "
                 "an image to kang it to your userbot pack.",
        'examples': ["{tr}keng", "{tr}kang Ã°ÂÂ¤Â", "{tr}kang 2", "{tr}kang Ã°ÂÂ¤Â 2"]},
...
async def kang_(message: Message):
    ...
    if photo:
        ...
        cmd = '/newpack'
        if resize:
            get_file = await resize_photo(photo)
        if is_anim:
            packname += "_anim"
            packnick += " (Animated)"
            cmd = '/newanimated'
            ...
            async with userge.conversation('Stickers', limit=30) as conv:
                ...
                while limit in msg.text:
                    ...
                    if msg.text == "Invalid pack selected.":
                        ...
                        await conv.get_response(mark_read=True)
                        await userge.forward_messages("stickers", "@EzStickerBot", get_file.message_id, as_copy=True)
                        await conv.get_response(mark_read=True)
...
@userge.on_cmd("stickerinfo", about={
...
async def resize_photo(photo_path: str) -> str:
    """ Resize the given photo to 512x512 """
    image = Image.open(photo_path)
    png_img = Config.DOWN_PATH + "sticker.png"
    image.save(png_img)
    async with userge.conversation("@EzStickerBot") as conv:
        try:
            await conv.send_message('/start')
        except YouBlockedUser:
            await message.err("Try to Unblock **@EzStickerBot**")
            return
        await conv.get_response(mark_read=True)
        await userge.send_photo("@EzStickerBot", png_img)
        resized_file = await conv.get_response(mark_read=True)
    os.remove(png_img)
    return resized_file
...
KANGING_STR = (
    ...
    "hehe me stel ur stikÃÂ©r\nhehe.",
    "Ay look over there (Ã¢ÂÂÃ¯Â½Â¡Ã¢ÂÂ)!Ã¢ÂÂ\nWhile I kang this...",
